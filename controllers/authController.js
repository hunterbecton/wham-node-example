...
const createSendToken = async (user, statusCode, req, res) => {
  ...
  res.cookie('whamRefresh', refreshToken, {
    httpOnly: true,
    sameSite: 'none',
...
exports.logout = catchAsync(async (req, res) => {
  ...
  res.cookie('whamRefresh', 'loggedout', {
    httpOnly: true,
    // sameSite: 'none',
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (req.cookies && req.cookies.whamJWT) {
    token = req.cookies.whamJWT;
  }

  if (req.cookies && req.cookies.whamRefresh) {
    refresh = req.cookies.whamRefresh;
  }
  ...
  if (!token && refresh) {
    try {
      ...
      res.cookie('whamJWT', refreshAuthToken, {
        httpOnly: true,
        sameSite: 'none',
        secure: req.secure || req.headers['x-forwarded-proto'] === 'https',
        maxAge: 1800000, // 30 minutes
