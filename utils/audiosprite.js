...
module.exports = function (files) {
  ...
  function makeRawAudioFile(src, cb) {
    ...
    fs.exists(src, function (exists) {
      if (exists) {
        ...
        let ffmpeg = spawn(
          'ffmpeg',
          ['-i', path.resolve(src)].concat(wavArgs).concat('pipe:')
        );
  ...
  function appendFile(name, src, dest, cb) {
    ...
    reader.on('close', function () {
      ...
      opts.logger.info('File added OK', {
        file: src,
  ...
  function appendSilence(duration, dest, cb) {
    var buffer = new Buffer(
      Math.round(opts.samplerate * 2 * opts.channels * duration)
    );
    buffer.fill(0);
  ...
  function exportFileCaf(src, dest, cb) {
    ...
    spawn('afconvert', ['-f', 'caff', '-d', 'ima4', src, dest]).on(
      'exit',
          ...
          appendFile(name, tmp, tempFile, function (err) {
            if (rawparts != null ? rawparts.length : void 0) {
              async.forEachSeries(
                rawparts,
                function (ext, cb) {
                  opts.logger.debug('Start export slice', {
                    name: name,
                    format: ext,
                    i: i,
                  });
        ...
        async.forEachSeries(
          ...
          function (err) {
            ...
            switch (opts.format) {
              ...
              case 'howler2':
                finalJson[
                  opts.format === 'howler' ? 'urls' : 'src'
                ] = [].concat(json.resources);
                finalJson.sprite = {};
